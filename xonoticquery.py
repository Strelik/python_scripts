...
"""Script that opens a UDP challenge packet to an active Xonotic server to collect info or could
...
class WarsowQuery:
    ...
    def __init__(self, ip, port=26000, timeout=1.0):
        self.ip = ip
        self.port = port
    ...
    def hostname_challenge(self):
        ...
        if packet:
            raw = packet.decode('iso-8859-1').split("\\")
            """The value for sv_hostname should be the 16th entry in the dictionary"""
            challenge = raw[16]
            return challenge
        return []#return empty dict if the connection attempt somehow failed.
