...
def main():
    ...
    w = XonoticQuery('95.156.230.70')

    heartbeat = w.hostname_challenge()

    """Test if the server instance name is returned. If not generate an email alert)"""
    if heartbeat == "[å] The Regulars - Instagib CTF + Hook [git]":
        pass
    else:
        smtpmessage.alert_message("Hey %s Xonotic server is down or not responding to heartbeat challenges. Fix it." % heartbeat)
